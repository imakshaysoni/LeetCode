...
public:
    ...
    int knightDialer(int n) {
        ...
        for(int digit=0;digit<10;digit++){
            ans =  (ans%mod + helper(n, m, digit, dp)%mod)%mod;
        }
    ...
    int helper(int n, unordered_map<int,vector<int>> &m, int digit, vector<vector<int>>&dp){
        ...
        for(int x: ms[digit]){
            sum= (sum%mod + helper(n-1,m,x,dp) % mod)%mod;
        }
